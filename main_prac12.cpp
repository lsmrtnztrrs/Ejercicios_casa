...
void display(void)   // Creamos la funcion donde se dibuja
{
	...
	//llantas
	glPushMatrix(); //llanta frontal derecha
	glTranslatef(-6, -1, 7.5);
	glRotatef(-rotTires, 1, 0, 0);
	llanta.GLrender(NULL, _SHADED, 1.0);
	glPopMatrix();

	glPushMatrix(); //llanta frontal izquierda
	glTranslatef(6, -1, 7.5);
	glRotatef(180, 0, 1, 0);
	glRotatef(rotTires, 1, 0, 0);
	llanta.GLrender(NULL, _SHADED, 1.0);
...
void animacion()
{
	...
	if (cto3) //comienza circuito
	{
		if (recorrido1)//comienza recorrido
		{
			movKitZ--;
			if (movKitZ < -155)
			{
		...
		if (recorrido3)//comienza recorrido 3
		{
			rotKit = 0;
			movKitZ++;
			if (movKitZ > 100)//cambia a 4
			{
				recorrido3 = false;
				recorrido4 = true;
			}
		}
		if (recorrido4)//empieza recorrido 4
		{
			rotKit = 0;
			movKitZ++;
			movKitY--;
			if (movKitY < 5)//cambia a 5
			{
				recorrido4 = false;
				recorrido5 = true;
			}
